...
export default function ReportModal({ isOpen, onClose, report, loading, account }) {
  ...
  function handleDownload() {
    ...
    const logoSrc = account?.avatar_url
      || (account?.district_domain ? `https://logo.clearbit.com/${account.district_domain}` : '')
    const win = window.open('', '_blank', 'width=920,height=800')
    win.document.write(`<!DOCTYPE html><html><head>
      <title>Quorum Report — ${acctData.name || ''}</title>
      <meta charset="utf-8" />
      <style>
              ...
              <>
                ...
                <div style={{ borderBottom: '2px solid #1A2158', paddingBottom: 18, marginBottom: 24 }}>
                  <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
                      {(account?.avatar_url || account?.district_domain) && (
                        <img
                          src={account?.avatar_url || `https://logo.clearbit.com/${account.district_domain}`}
                          alt={acctData.name}
                      ...
                      <div>
                        ...
                        <div style={{ display: 'flex', gap: 8, marginTop: 8, flexWrap: 'wrap' }}>
                          ...
                          }}>{acctData.account_type === 'client' ? 'Partner District' : 'Prospect'}</span>
                        </div>
                      </div>
                    </div>
                ...
                <div style={{ marginTop: 32, paddingTop: 16, borderTop: '1px solid #E5E7EB',
                  ...
                  <span style={{ fontSize: 9, fontFamily: 'var(--mono)', color: '#B0BCDA', letterSpacing: '0.08em' }}>
                    GENERATED BY QUORUM — EdTech Intelligence
                  </span>
